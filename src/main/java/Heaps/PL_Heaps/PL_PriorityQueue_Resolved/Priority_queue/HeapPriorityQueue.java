 ...
 */
package Heaps.PL_Heaps.PL_PriorityQueue_Resolved.Priority_queue;
...
public class HeapPriorityQueue<K extends Comparable,V> extends AbstractPriorityQueue<K,V> {
  ...
  public void NLargestNumbers(HeapPriorityQueue<K,V> hp, int n){
    if (size() <= n)  return;
    while (size() > n) removeMin();
